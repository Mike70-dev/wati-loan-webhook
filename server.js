...
function cleanNumber(value) {
  ...
  // If both dot and comma exist, assume European style like 7.500,50
  if (str.includes(".") && str.includes(",")) {
    str = str.replace(/\./g, "");
    str = str.replace(",", ".");
  }
  // If only dot exists, decide if it is thousands separator
  else if (str.includes(".")) {
    if (/^\d{1,3}(\.\d{3})+$/.test(str)) {
      str = str.replace(/\./g, "");
    }
  }
  // If only comma exists, decide if it is thousands separator or decimal separator
  else if (str.includes(",")) {
    if (/^\d{1,3}(,\d{3})+$/.test(str)) {
      str = str.replace(/,/g, "");
    } else {
...
Die monatliche Rate muss größer als 0 sein.`
  ...
  for (let year = minYears; year <= maxYears; year++) {
    ...
    // Only accept offers that do not exceed desired monthly payment
    if (monthlyPayment <= desiredMonthly) {
      // Choose the highest possible monthly payment that is still affordable
...
app.get("/", (req, res) => {
  res.send(`
    ...
    <head>
      ...
      <title>Loan Calculator Tool</title>
      <style>
        body {
...
app.post("/calculate-loan", (req, res) => {
  try {
    ...
    // Extract years and monthly rate again from result.message would be ugly,
    // so recalculate for structured output:
    const minYears = 1;
    const maxYears = 25;
    const yearlyRate = 0.025;
